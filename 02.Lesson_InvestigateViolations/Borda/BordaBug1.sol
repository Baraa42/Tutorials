...
contract Borda is IBorda {
    ...
    function getPointsOfContender(address contender)
        public
        view
        override
        returns (uint256)
        ...
        )
    {
        ...
        return (
            contender_details.age,
            contender_details.registered,
            contender_details.points
        );
    }
    ...
    function registerContender(uint8 age) external override returns (bool) {
        require(
            !_contenders[msg.sender].registered,
            "you are already registered"
        );
        _contenders[msg.sender] = Contenders({
            age: age,
            registered: true,
            points: 0
        });
    ...
    function vote(
        ...
        require(
            _voters[msg.sender].registered,
            "you are not registered. before you vote you have to register yourself"
        );
        require(
            _contenders[first].registered &&
                _contenders[second].registered &&
                _contenders[third].registered,
            "one or more of the specified addresses aren't registered as contenders"
        );
        require(
            first != second && first != third && second != third,
            "you've tried to vote for the same more than once"
        );

        Voters memory voter_details = _voters[msg.sender];
        _voters[msg.sender].vote_attempts = voter_details.vote_attempts.safeAdd(
            1
        );
        ...
        voteTo(first, 3);
        voteTo(second, 2); //@note: should add 2, was adding 3.
        voteTo(third, 1); //@note: should add 2, was adding 1.

        return true;
    }

    function voteTo(address contender, uint256 points) private {
        uint256 contender_points = _contenders[contender].points.safeAdd(
            points
        );
